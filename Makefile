...
ASFLAGS  := -g $(ARCH)
LDFLAGS  := -specs=$(DEVKITPRO)/libnx/switch.specs -g $(ARCH) -Wl,-Map,$(notdir $*.map) -Wl,--wrap=main -Wl,--wrap=nce_diag_set_run_id -Wl,--wrap=vsnprintf -Wl,--wrap=_svfprintf_r -Wl,--wrap=_localeconv_r -Wl,--wrap=strlen -Wl,--wrap=memset -Wl,--wrap=__getreent -Wl,--wrap=__locale_mb_cur_max -Wl,--wrap=_malloc_r
LIBS     := -lnx
