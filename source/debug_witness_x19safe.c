...
extern uint32_t nce_diag_emit_debug_witness_x19(
    const char *text,
    uint64_t length,
    uint64_t witness);
...
static uint64_t combined_tag(const char *text, uint64_t length) {
    ...
    return NCE_DIAG_WITNESS_TAG_PREFIX | (uint64_t)hash;
}

uint32_t nce_diag_output_debug_string_x19safe(const char *text, uint64_t length) {
    return nce_diag_emit_debug_witness_x19(text, length, combined_tag(text, length));
}
